...
project {
    ...
    params {
        ...
        text("github_repository_name", "%projectName%", label = "The github repository name. Used to connect to it in VCS Roots.", description = "This is the repository slug on github. So for example `%projectName%` or `MinecraftForge`. It is interpolated into the global VCS Roots.", display = ParameterDisplay.HIDDEN, allowEmpty = false)
        text("env.PUBLISHED_JAVA_ARTIFACT_ID", "%projectArtifactId%", label = "Published artifact id", description = "The maven coordinate artifact id that has been published by this build. Can not be empty.", allowEmpty = false)
        text("env.PUBLISHED_JAVA_GROUP", "%projectArtifactGroup%", label = "Published group", description = "The maven coordinate group that has been published by this build. Can not be empty.", allowEmpty = false)
        text("docker_jdk_version", "%jdkVersion%", label = "JDK version", description = "The version of the JDK to use during execution of tasks in a JDK.", display = ParameterDisplay.HIDDEN, allowEmpty = false)
        text("docker_gradle_version", "%gradleVersion%", label = "Gradle version", description = "The version of Gradle to use during execution of Gradle tasks.", display = ParameterDisplay.HIDDEN, allowEmpty = false)
    }

    features {
        githubIssues {
            id = "%projectName%__IssueTracker"
            displayName = "%projectOrg%/%projectName%"
            repositoryURL = "https://github.com/%projectOrg%/%projectName%"
        }
    }
}
...
object BuildSecondaryBranches : BuildType({
    ...
    id("%projectName%__BuildSecondaryBranches")
